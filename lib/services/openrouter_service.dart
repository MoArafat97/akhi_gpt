...
Remember: You're here to be a supportive companion, but respect goes both ways. Stay calm, be firm when needed, and always prioritize creating a safe, respectful space for meaningful conversation.''';
  ...
  Future<String?> get _apiKey async {
    // Use backend API key from .env file
    final envApiKey = dotenv.env['OPENROUTER_API_KEY'];
    return (envApiKey != null && envApiKey.isNotEmpty) ? envApiKey : null;
  }

  /// Get the current active model from environment configuration
  Future<String> get _currentModel async {
    // Try to get the last working model from storage first
    ...
    // Fallback to a hardcoded model if environment is not configured
